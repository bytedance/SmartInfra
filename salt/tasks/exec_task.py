...
from salt.models import host_group, host_group_minion, hosts, salt_master, task_list, shell_template, user_relationship, resource_group, rg_relationship, User
...
def exec_remote_shell(*args, **kwargs):
    ...
    try:
        ...
        for each_minion in all_minions:
            ...
            # check that if current user have permission to operate this minion
            if not hosts.objects.filter(name=each_minion, salt__in=current_user_salt_object):
                count_fail += 1
                cleaned_content = "no permission"
